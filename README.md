...
- **var.py** — Centralizes configuration values.
...
Run for workspace creation:

```bash
python create_workspace.py
```

Run for dependency cleanup:

```bash
python clean_dependencies.py
```

If you need to change settings, edit `var.py`:

- `ROOT_PATH` — workspace root containing the `Projects` folder.
- `FOLDERS_TO_DELETE` — dependency/cache directory names to remove.
- `EXCLUDED_PATHS` — relative project paths to skip.
